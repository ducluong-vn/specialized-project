...
import { showerAdded, showerRemoved, selectAllShower } from './showerSlice'
...
export default function TimeListshower({ title, feed_id, feed_value }) {
    const showers = useSelector(selectAllShower)
    ...
    const onSavePostClicked = () => {
        if (hour && minute) {
            dispatch(showerAdded(hour, minute))
            setHour('')
            setMinute('')
            setOpen(false);
        }
    ...
    return <div className={styles.container}>
        ...
        <Grid container spacing={2}>
            {showers.map(item => (<Grid item xs={12} key={item.id}>
